...
class CFDMAEDetector(nn.Module):
    ...
    def __init__(self, num_classes, pretrained_cfdmae_path=None,
        ...
        if dasm_replacement != 'dasm':
            raise ValueError("This reproducibility release keeps only the main DASM module.")
        self.dasm_replacement = 'dasm'
        self.use_hf_prior = self.use_hf
        ...
        if yolo_pretrained_path and 'yolo26' not in yolo_pretrained_path.lower():
            raise ValueError("CFD-MAE release uses YOLOv26. Set YOLO_PRETRAINED_PATH to a YOLOv26 weight.")
